...
use Exception;
use Modules\Dropbox\Entities\DropboxSetting;
...
class Dropbox
{
    ...
    public function getAccessToken($returnNullNoAccessToken = null)
    {
        ...
        // dd(json_decode(company_setting('dropbox_token_data')));
        $token = json_decode(company_setting('dropbox_token_data'));
        ...
        if (isset($token->refresh_token)) {
            ...
            if ($token->expires_in <= $now) {
                ...
                $post['dropbox_token_data']    = json_encode($data);

                $check = DropboxSetting::saveSettings($post);
