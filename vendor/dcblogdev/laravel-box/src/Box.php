...
use Exception;
use Modules\Box\Entities\BoxSetting;
...
class Box
{
    ...
    protected function storeToken($access_token, $refresh_token, $expires, $token=''): void
    {
        ...
        $post['box_tokens_data']    = json_encode($token);
        $post['box_access_token']   = $access_token;
        $post['box_refresh_token']  = $refresh_token;
        $post['box_expires']        = $expires;

        $check = BoxSetting::saveSettings($post);
