...
abstract class Client implements AuthenticationInterface
{
        ...
        $settings)
    {
        ...
        if(session('forrest_token_salesforce') != getActiveWorkSpace()){
            session()->forget(['forrest_token', 'forrest_refresh_token','code_verifier','forrest_version','forrest_resources']);
        }

        if(!session()->has(['forrest_token', 'forrest_refresh_token','code_verifier','forrest_version','forrest_resources']) && (company_setting('salesforce_token') !== null))
        {
            $response = (array) json_decode(company_setting('salesforce_token'));
        ...
        $data = ['forrest_token_salesforce' => getActiveWorkSpace()];
        session()->forget('forrest_token_salesforce');
        session()->put($data);
    ...
    {
        return company_setting('salesforce_token');
