    ...
    <script>
        $(document).on('change', "[name='retainer_type_radio']", function() {
            ...
            if (val == 'product') {
                ...
                var label =
                    ` {{ Form::label('project', __('Project'), ['class' => 'form-label']) }} {{ Form::select('project', $projects, $retainer->category_id, ['class' => 'form-control', 'required' => 'required']) }}`
                $(".retainer_div").append(label);
...
@section('content')
    <div class="row">
        {{ Form::model($retainer, ['route' => ['retainer.update', $retainer->id], 'method' => 'PUT', 'enctype' => 'multipart/form-data', 'class' => 'w-100']) }}
                    ...
                    <div class="row">
                            ...
                            <div class="row" id="customer-box">
                                <div class="form-group col-md-6" id="account-box">
                                    ...
                                    <select
                                        ...
                                        <option value="Accounting" @if ($retainer->account_type == 'Accounting') selected @endif>
                                            {{ __('Accounting') }}</option>
                                        <option value="Projects" @if ($retainer->account_type == 'Projects') selected @endif>
                                            {{ __('Projects') }}</option>
                                    </select>
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    {{ Form::select('customer_id', $customers, !empty($retainer->user_id) ? $retainer->user_id : null, ['class' => 'form-control choices ', 'id' => 'customer', 'data-url' => route('retainer.customer'), 'required' => 'required', 'placeholder' => 'Select Customer']) }}
                        ...
                        <div class="col-md-6">
                            <div class="row">
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                                ...
                                                <select
                                                    ...
                                                    name="retainer_type_radio" required="" id="billing_type">
                                                    <option value="product"
                                                        @if ($retainer->retainer_module == 'account') selected @endif>
                                                        {{ __('Item Wise') }}</option>
                                                    <option value="project"
                                                        @if ($retainer->retainer_module == 'taskly') selected @endif>
                               ...
                               <div class="col-md-6">
                                <div class="form-group">
                                    <label for="retainer_template" class="form-label">{{ __('Template') }}</label>
                                    <select class="form-control" name="retainer_template" id="retainer_template">
                                        ...
                                        @foreach (templateData()['templates'] as $key => $template)
                                            <option value="{{ $key }}"
                                                {{ $retainer->retainer_template == $key ? 'selected' : '' }}>
                                                {{ $template }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group retainer_div">
                                        ...
                                        @elseif (module_is_active('Taskly'))
                                            ...
                                            {{ Form::select('project', $projects, $retainer->category_id, ['class' => 'form-control ', 'required' => 'required']) }}
                                        @endif
                                    </div>
                                </div>
                                <div class="col-md-6">
                                ...
                                @if (module_is_active('Taskly'))
                                    <div
                                        ...
                                        <div class="form-group">
                                            ...
                                            {{ Form::select('tax_project[]', $taxs, !empty($retainer->items->first()->tax) ? explode(',', $retainer->items->first()->tax) : null, ['class' => 'form-control get_tax multi-select choices', 'multiple' => 'multiple', 'id' => 'tax_project']) }}
                                    ...
                                    <div class="col-md-12">
                                        <div class="tab-pane fade show" id="tab-2" role="tabpanel">
                                            @include('customfield::formBuilder', [
                                                'fildedata' => $retainer->customField,
                                            ])
                                        </div>
                                    </div>
                                @endif
        ...
        <div class="modal-footer">
            <input type="button" value="{{ __('Cancel') }}" onclick="location.href = '{{ route('retainer.index') }}';"
                class="btn btn-light">
            <input type="submit" id="submit" value="{{ __('Save Changes') }}" class="btn btn-primary mx-3">
        </div>
        {{ Form::close() }}
    </div>
@endsection
