...
class CommonCaseController extends Controller
{
    ...
    public function index()
    {
        ...
        {
            $cases = CommonCase::with('accounts','assign_user')->where('created_by', creatorId())->where('workspace',getActiveWorkSpace())->get();
    ...
    public function create($type, $id)
    {
        if(\Auth::user()->isAbleTo('case create'))
        {
            ...
            if(module_is_active('CustomField')){
                $customFields =  \Modules\CustomField\Entities\CustomField::where('workspace_id',getActiveWorkSpace())->where('module', '=', 'sales')->where('sub_module','Case')->get();
            }else{
                $customFields = null;
            }
    ...
    public function store(Request $request)
    {
        ...
        if(\Auth::user()->isAbleTo('case create'))
        {
            ...
            if(module_is_active('CustomField'))
            {
                \Modules\CustomField\Entities\CustomField::saveData($commoncase, $request->customField);
            }
    ...
    public function show($id)
    {
        if(\Auth::user()->isAbleTo('case show'))
        {
            ...
            if(module_is_active('CustomField')){
                $commonCase->customField = \Modules\CustomField\Entities\CustomField::getData($commonCase, 'Sales','Case');
                $customFields      = \Modules\CustomField\Entities\CustomField::where('workspace_id', '=', getActiveWorkSpace())->where('module', '=', 'Sales')->where('sub_module','Case')->get();
            }else{
                $customFields = null;
            }
    ...
    public function destroy($id)
    {
        if(\Auth::user()->isAbleTo('case delete'))
        {
            ...
            if(module_is_active('CustomField'))
            {
                $customFields = \Modules\CustomField\Entities\CustomField::where('module','sales')->where('sub_module','Case')->get();
                foreach($customFields as $customField)
                {
                    $value = \Modules\CustomField\Entities\CustomFieldValue::where('record_id', '=', $commonCase->id)->where('field_id',$customField->id)->first();
                    if(!empty($value)){
                        $value->delete();
                    }
                }
